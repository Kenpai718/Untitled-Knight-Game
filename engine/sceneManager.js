    ...
    loadScene(level, x, y) {
        ...
        if (this.level.platforms) {
            for (var i = 0; i < this.level.platforms.length; i++) {
                ...
                this.game.addEntity(new Platform(this.game, platforms.x * PARAMS.BLOCKDIM, platforms.y * PARAMS.BLOCKDIM, platforms.width * PARAMS.BLOCKDIM, platforms.height * PARAMS.BLOCKDIM));
            }
        }
        if (this.level.walls) {
